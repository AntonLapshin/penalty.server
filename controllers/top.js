...
function update(id, score, goals, miss, exp) {
    return $.Deferred(function(defer){
        DB.getTop().update({ _id: id }, { score: score, last: Date.now(), exp: exp, goals: goals, miss: miss }, function(err, value){
            if (err)
                defer.reject(err);
            else
                defer.resolve(value);
...
function insert(id, score, goals, miss, exp) {
    return $.Deferred(function(defer){
        DB.getTop().insert({ _id: id, score: score, last: Date.now(), exp: 1, goals: goals, miss: miss }, function(err, value){
            if (err)
                defer.reject(err);
            else
                defer.resolve(value);
...
function exists(id) {
    return $.Deferred(function(defer){
        DB.getTop().findOne({ _id: id }, { _id: 1}, function(err, value){
            if (err)
                defer.reject(err);
            else
                defer.resolve(value);
    ...
    return $.Deferred(function(defer){
        DB.getTop()
            .find(
                ...
                { _id: 1, score: 1, exp: 1, goals: 1, miss: 1 }
            )
    ...
    return $.Deferred(function(defer) {
        DB.getTop()
            .find(
                ...
                { _id: 1, score: 1, exp: 1, goals: 1, miss: 1, last: 1 }
            )
            .toArray(function(err, value){
                if (err)
                    defer.reject(err);
